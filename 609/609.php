<!DOCTYPE html >

<html lang="en">
<head>
    <title>758</title>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="css/font-awesome.min.css" rel="stylesheet" type="text/css" />
    <link href="css/bootstrap.min.css" rel="stylesheet" type="text/css" />
    <link href="css/758.css" rel="stylesheet" type="text/css" />
    <?php
    if (!class_exists('lessc')) {
        include ('./libs/lessc.inc.php');
    }
    $less = new lessc;
    $less->compileFile('less/758.less', 'css/758.css');
    ?>
    <script src="js/jquery-3.1.1.min.js"></script>
    <script src="js/bootstrap.min.js" ></script>
    <script src="js/scripts-categories-1.js" ></script>
    <script src="js/scripts-categories-2.js" ></script>
</head>

<body>
    <div class="type-758">
        <div class="header">
            <div class="container">
                <div id="switch">
                    <div class="switch-money">
                        <div class="dropdown">
                            <button class="dropdown-toggle" type="button" data-toggle="dropdown">$
                                <span class="caret"></span>
                            </button>
                            <ul class="dropdown-menu pull-right">
                                <li class="item-topmenu"><a href="#">€ Euro</a></li>
                                <li class="item-topmenu"><a href="#">£ Pound Sterling</a></li>
                                <li class="item-topmenu"><a href="#">$ US Dollar</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="switch-language">
                        <div class="dropdown">
                            <button class="dropdown-toggle" type="button" data-toggle="dropdown">En
                                <span class="caret"></span></button>
                                <ul class="dropdown-menu pull-right">
                                    <li class="item-topmenu"><a href="#">English</a></li>
                                    <li class="item-topmenu"><a href="#">Deutsch</a></li>
                                    <li class="item-topmenu"><a href="#">Русский</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="container">
                    <div class="row">
                        <div class="col-md-3 col-sm-4 col-xs-6 mobile">
                            <figure id="logo">
                                <a href="#"><img  src="images/logo.png" alt=""></a>
                            </figure>
                        </div>
                        <div class="col-md-6 col-sm-5 col-xs-6 mobile">
                            <div id="search">
                                <input type="text" placeholder="Search"><button type="button" class="button-search">Search</button>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div id="phone">
                                <a href="#"><i class="fa fa-phone"></i></a>
                                <span>800-2345-6789</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="type-L">
                <div class="type-34">
                    <div class="container">
                        <div class="col-md-3">
                            <div class="row">
                                <div class="type-L-home-categories">
                                    <div class="mod-custom-21">               
                                        <div class="container">
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <div class="box category">
                                                        <div class="box-heading"><h3><i class="fa fa-bars" aria-hidden="true"></i>Categories</h3></div>

                                                        <div class="box-category">
                                                            <ul class="cate">
                                                                <li>
                                                                    <a href="">Sound devices<i class="fa fa-angle-right"></i></a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Video game consoles</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Cell/mobile/wireless phones</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Home security systems</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Cameras</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Home theater systems</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">TVs</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Computers</a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">Games/movies/music</a>
                                                                    <li>
                                                                        <a href="#">Accessories</a>
                                                                    </li>
                                                                    <li>
                                                                        <a href="#">Office</a>
                                                                    </li>
                                                                    <li>
                                                                        <a href="#">Home Appliances</a>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="cate">
                                                            <div class="carousel " >
                                                                <h3>Specials</h3>
                                                                <br>
                                                                <div class="ab"> 
                                                                    <a class="prev"><span class="glyphicon glyphicon-chevron-up"></span></a>
                                                                    <a class="next"><span class="glyphicon glyphicon-chevron-down"></span></a>
                                                                </div>
                                                                <br>
                                                                <div class="window">
                                                                    <ul >     
                                                                        <li class="item">
                                                                            <div class="sale">Sale</div><br>
                                                                            <div class="aa "><div class="row">
                                                                                <div class="col-xs-12 col-md-3" >
                                                                                    <figure class="image">
                                                                                        <img  src="images/product-55-760x550.png" alt="" />
                                                                                    </figure>
                                                                                </div>
                                                                                <div class="col-xs-12 col-md-8">
                                                                                    <div class="caption">
                                                                                        <div class="price">
                                                                                            <span class="price-new">
                                                                                            $80.00 </span>
                                                                                            <span class="price-old">
                                                                                            $1,300.00 </span>
                                                                                        </div>
                                                                                        <div class="name">
                                                                                            <a href="">
                                                                                            EOS 70D Digital SLR Camera </a>
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </li>

                                                                    <li class="item">
                                                                        <div class="sale">Sale</div><br>
                                                                        <div class="aa "><div class="row">
                                                                            <div class="col-xs-12 col-md-3" >
                                                                                <figure class="image">
                                                                                    <img  src="images/product-52-760x550.png" alt="" />
                                                                                </figure>
                                                                            </div>
                                                                            <div class="col-xs-12 col-md-8">
                                                                                <div class="caption">
                                                                                    <div class="price">
                                                                                        <span class="price-new">
                                                                                        $80.00 </span>
                                                                                        <span class="price-old">
                                                                                        $1,300.00 </span>
                                                                                    </div>
                                                                                    <div class="name">
                                                                                        <a href="">
                                                                                        EOS 70D Digital SLR Camera </a>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </li>
                                                                <li class="item">
                                                                    <div class="sale">Sale</div><br>
                                                                    <div class="aa "><div class="row">
                                                                        <div class="col-xs-12 col-md-3" >
                                                                            <figure class="image">
                                                                                <img  src="images/product-7-760x550.png" alt="" />
                                                                            </figure>
                                                                        </div>
                                                                        <div class="col-xs-12 col-md-8">
                                                                            <div class="caption">
                                                                                <div class="price">
                                                                                    <span class="price-new">
                                                                                    $80.00 </span>
                                                                                    <span class="price-old">
                                                                                    $1,300.00 </span>
                                                                                </div>
                                                                                <div class="name">
                                                                                    <a href="">
                                                                                    EOS 70D Digital SLR Camera </a>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </li>

                                                            <li class="item">
                                                                <div class="sale">Sale</div><br>
                                                                <div class="aa"><div class="row">
                                                                    <div class="col-xs-12 col-md-3">
                                                                        <figure class="image">
                                                                            <img  src="images/product-34-760x550.png" alt="" />
                                                                        </figure>
                                                                    </div>
                                                                    <div class="col-xs-12 col-md-8">
                                                                        <div class="caption">
                                                                            <div class="price">
                                                                                <span class="price-new">
                                                                                $80.00 </span>
                                                                                <span class="price-old">
                                                                                $1,300.00 </span>
                                                                            </div>
                                                                            <div class="name">
                                                                                <a href="">
                                                                                EOS 70D Digital SLR Camera </a>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <li class="item">
                                                                <div class="sale">Sale</div><br>
                                                                <div class="aa"><div class="row">
                                                                    <div class="col-xs-12 col-md-3">
                                                                        <figure class="image">
                                                                            <img  src="images/product-52-760x550.png" alt="" />
                                                                        </figure>
                                                                    </div>
                                                                    <div class="col-xs-12 col-md-8">
                                                                        <div class="caption">
                                                                            <div class="price">
                                                                                <span class="price-new">
                                                                                $80.00 </span>
                                                                                <span class="price-old">
                                                                                $1,300.00 </span>
                                                                            </div>
                                                                            <div class="name">
                                                                                <a href="">
                                                                                EOS 70D Digital SLR Camera </a>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </li>
                                                        <li class="item">
                                                            <div class="sale">Sale</div><br>
                                                            <div class="aa"><div class="row">
                                                                <div class="col-xs-12 col-md-3">
                                                                    <figure class="image">
                                                                        <img  src="images/product-55-760x550.png" alt="" />
                                                                    </figure>
                                                                </div>
                                                                <div class="col-xs-12 col-md-8">
                                                                    <div class="caption">
                                                                        <div class="price">
                                                                            <span class="price-new">
                                                                            $80.00 </span>
                                                                            <span class="price-old">
                                                                            $1,300.00 </span>
                                                                        </div>
                                                                        <div class="name">
                                                                            <a href="">
                                                                            EOS 70D Digital SLR Camera </a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </li>
                                                    <li class="item">
                                                        <div class="sale">Sale</div><br>
                                                        <div class="aa"><div class="row">
                                                            <div class="col-xs-12 col-md-3">
                                                                <figure class="image">
                                                                    <img  src="images/product-34-760x550.png" alt=""/>
                                                                </figure>
                                                            </div>
                                                            <div class="col-xs-12 col-md-8">
                                                                <div class="caption">
                                                                    <div class="price">
                                                                        <span class="price-new">
                                                                        $80.00 </span>
                                                                        <span class="price-old">
                                                                        $1,300.00 </span>
                                                                    </div>
                                                                    <div class="name">
                                                                        <a href="">
                                                                        EOS 70D Digital SLR Camera </a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </li>
                                            </ul>
                                        </div> <!-- .window -->
                                    </div> 
                                </div>
                            </div>
                        </div>                               
                    </div>
                </div>
                <div class="type-21">               
                    <div class="container">
                        <div class="row">
                            <div class=" col-md-3">
                                <div class="cate">
                                    <div class="carousels ">

                                        <h3>Specials</h3>
                                        <br>
                                        <div class="ab"> 
                                            <a class="prev"><span class="glyphicon glyphicon-chevron-up"></span></a>
                                            <a class="next"><span class="glyphicon glyphicon-chevron-down"></span></a>
                                        </div>
                                        <div class="window">
                                            <ul >     
                                                <li class="item">

                                                    <div class="aa "><div class="row">
                                                        <div class="col-xs-12 col-md-3" >
                                                            <figure class="image">
                                                                <img  src="images/product-55-760x550.png" alt="" />
                                                            </figure>
                                                        </div>
                                                        <div class="col-xs-12 col-md-8">
                                                            <div class="caption">
                                                                <div class="price">
                                                                    <span class="price-new">
                                                                    $80.00 </span>
                                                                    <span class="price-old">
                                                                    $1,300.00 </span>
                                                                </div>
                                                                <div class="name">
                                                                    <a href="">
                                                                    EOS 70D Digital SLR Camera </a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </li>

                                            <li class="item">

                                                <div class="aa "><div class="row">
                                                    <div class="col-xs-12 col-md-3">
                                                        <figure class="image">
                                                            <img  src="images/product-52-760x550.png" alt=""  />
                                                        </figure>
                                                    </div>
                                                    <div class="col-xs-12 col-md-8">
                                                        <div class="caption">
                                                            <div class="price">
                                                                <span class="price-new">
                                                                $80.00 </span>
                                                                <span class="price-old">
                                                                $1,300.00 </span>
                                                            </div>
                                                            <div class="name">
                                                                <a href="">
                                                                EOS 70D Digital SLR Camera </a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                                        <li class="item">

                                            <div class="aa "><div class="row">
                                                <div class="col-xs-12 col-md-3" >
                                                    <figure class="image">
                                                        <img  src="images/product-7-760x550.png" alt="" />
                                                    </figure>
                                                </div>
                                                <div class="col-xs-12 col-md-8">
                                                    <div class="caption">
                                                        <div class="price">
                                                            <span class="price-new">
                                                            $80.00 </span>
                                                            <span class="price-old">
                                                            $1,300.00 </span>
                                                        </div>
                                                        <div class="name">
                                                            <a href="">
                                                            EOS 70D Digital SLR Camera </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>

                                    <li class="item">

                                        <div class="aa"><div class="row">
                                            <div class="col-xs-12 col-md-3">
                                                <figure class="image">
                                                    <img  src="images/product-55-760x550.png" alt="" />
                                                </figure>
                                            </div>
                                            <div class="col-xs-12 col-md-8">
                                                <div class="caption">
                                                    <div class="price">
                                                        <span class="price-new">
                                                        $80.00 </span>
                                                        <span class="price-old">
                                                        $1,300.00 </span>
                                                    </div>
                                                    <div class="name">
                                                        <a href="">
                                                        EOS 70D Digital SLR Camera </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <li class="item">

                                        <div class="aa"><div class="row">
                                            <div class="col-xs-12 col-md-3">
                                                <figure class="image">
                                                    <img  src="images/product-52-760x550.png" alt="" />
                                                </figure>
                                            </div>
                                            <div class="col-xs-12 col-md-8">
                                                <div class="caption">
                                                    <div class="price">
                                                        <span class="price-new">
                                                        $80.00 </span>
                                                        <span class="price-old">
                                                        $1,300.00 </span>
                                                    </div>
                                                    <div class="name">
                                                        <a href="">
                                                        EOS 70D Digital SLR Camera </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li class="item">

                                    <div class="aa"><div class="row">
                                        <div class="col-xs-12 col-md-3">
                                            <figure class="image">
                                                <img  src="images/product-7-760x550.png" alt=""/>
                                            </figure>
                                        </div>
                                        <div class="col-xs-12 col-md-8">
                                            <div class="caption">
                                                <div class="price">
                                                    <span class="price-new">
                                                    $80.00 </span>
                                                    <span class="price-old">
                                                    $1,300.00 </span>
                                                </div>
                                                <div class="name">
                                                    <a href="">
                                                    EOS 70D Digital SLR Camera </a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                            <li class="item">

                                <div class="aa"><div class="row">
                                    <div class="col-xs-12 col-md-3">
                                        <figure class="image">
                                            <img src="images/product-55-760x550.png" alt="" />
                                        </figure>
                                    </div>
                                    <div class="col-xs-12 col-md-8">
                                        <div class="caption">
                                            <div class="price">
                                                <span class="price-new">
                                                $80.00 </span>
                                                <span class="price-old">
                                                $1,300.00 </span>
                                            </div>
                                            <div class="name">
                                                <a href="">
                                                EOS 70D Digital SLR Camera </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div> <!-- .window -->
            </div> 
        </div>
    </div>
</div>                               
</div>
</div>
</div>
</div>
</div>

<div class="col-md-9 col-xs-12">
    <div class="row">
        <div class="type-L-4">
            <nav class="navbar navbar-default">
                <div class="container-fluid">
                    <div class="navbar-header">
                        <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
                            <span class="sr-only">Toggle navigation</span>
                            <span class="icon-bar"></span>
                            <span class="icon-bar"></span>
                            <span class="icon-bar"></span>
                        </button>
                    </div>
                    <div id="navbar" class="navbar-collapse collapse">
                        <ul class="nav navbar-nav">
                            <li class="bg"><a href="#"><i class="glyphicon glyphicon-home"></i><span>Home</span></a></li>
                            <li class="bg"><a href="#"><i class="glyphicon glyphicon-star-empty"></i><span>Wish List</span></a></li>
                            <li class="bg dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="glyphicon glyphicon-user"></i><span>My Account</span><span class="caret"></span></a>
                                <ul class="item dropdown-menu">
                                    <li class="subitem"><a href="#" >REGISTER</a></li>
                                    <li class="subitem"><a href="#" >LOGIN</a></li>
                                </ul>
                            </li>
                            <li class="bg"><a href="#"><i class="glyphicon glyphicon-shopping-cart"></i><span>Shopping Cart</span></a></li>
                            <li class="bg last"><a href="#"><i class="glyphicon glyphicon-check"></i><span>Check</span></a></li>
                            <li class="cart dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="glyphicon glyphicon-shopping-cart"></i> 0 item(s) <span class="caret"></span></a>
                                <ul class="cart-item dropdown-menu">
                                    <li class="cart-subitem">0 item(s)</li>
                                </ul>
                            </li>
                        </ul>
                    </div><!--/.nav-collapse -->
                </div><!--/.container-fluid -->
            </nav>
        </div>
    </div>

    <div class="clearfix"></div>

    <div class="row">
        <div class="banner">

            <div id="myCarousel" class="carousel slide">
                <div class="carousel-inner">
                    <div class="item active">
                        <div class="col-md-6">
                            <figure class="row">
                                <img  src="images/iphone-6.png" alt=""/>
                            </figure>
                        </div>
                        <div class="col-md-6">
                            <div class="carousel">
                                <div class="top-right">
                                    <h1> iPhone 6 </h1>
                                </div>
                                <div class="center-right">
                                    <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p>
                                </div>
                                <div class="btn-shop">
                                    <a href="#"> Shop now! </a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="item">
                        <div class="col-md-6">
                            <figure class="row">
                                <img src="images/tv-smart.png" alt=""/>
                            </figure>
                        </div>
                        <div class="col-md-6">
                            <div class="right">
                                <div class="top-right">
                                    <h1> Smart TV </h1>
                                </div>
                                <div class="center-right">
                                    <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p>
                                </div>
                                <div class="btn-shop">
                                    <a href="#"> Shop now! </a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="item">
                        <div class="col-md-6">
                            <figure class="row">
                                <img  src="images/ipad.png" alt=""/>
                            </figure>
                        </div>
                        <div class="col-md-6">
                            <div class="right">
                                <div class="top-right">
                                    <h1> iPad </h1>
                                </div>
                                <div class="center-right">
                                    <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p>
                                </div>
                                <div class="btn-shop">
                                    <a href="#"> Shop now! </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
        <div class="btn-slide">
            <ul>
                <li><a href="#myCarousel" data-slide="prev"><span class="glyphicon glyphicon-chevron-left"></span></a></li>
                <li><a href="#myCarousel" data-slide="next"><span class="glyphicon glyphicon-chevron-right"></span></a></li>
            </ul>
        </div>
    </div>

    <div class="clearfix"></div>

    <div class="content">
        <div class="row">
            <div class="top-content">
                <div class="col-md-6">
                    <div class="row">
                        <div class="left-top">
                            <div class="col-md-6">
                                <div class="row"> 
                                    <div class="info">
                                        <h2> TV & Video</h2>
                                        <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p>
                                        <a href="#"> Shop now! </a>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-6">
                                <div class="row">
                                    <figure class="image">
                                        <img  src="images/left-top-content.png" alt="" class="img-responsive"/>
                                    </figure>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-md-6">
                    <div class="row">
                        <div class="right-top">
                            <div class="col-md-6">
                                <div class="row"> 
                                    <div class="info">
                                        <h2> Home appliances </h2>
                                        <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p>
                                        <a href="#"> Shop now! </a>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-6">
                                <div class="row">
                                    <figure class="image">
                                        <img  src="images/right-top-content.png" alt=""/>
                                    </figure>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!--Center Content>
            Lastest-->
            <div class="row">
                <div class="center-content">
                    <div class="row">
                        <div class="nav">
                            <h2> Lastest </h2>
                            <ul>
                                <li><a href="#myLastest" data-slide="prev"><span class="glyphicon glyphicon-chevron-up"></span> </a></li>
                                <li><a href="#myLastest" data-slide="next"><span class="glyphicon glyphicon-chevron-down"> </span></a></li>
                            </ul>
                        </div>
                    </div>

                    <div id="myLastest" class="carousel slide">
                        <div class="carousel-inner">
                            <div class="item active">
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left:15px;">
                                            <div class="new">
                                                <a href="#"> New </a>
                                            </div>
                                            <figure class="images">
                                                <img  src="images/master-iron.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $40.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> Master Iron width Anti-Drip-System</a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active1"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <div class="new">
                                                <a href="#"> New </a>
                                            </div>
                                            <figure class="images">
                                                <img src="images/48-inch.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $550.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> 48-Inch 1080p 60Hz Smart LED TV </a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active2"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <div class="new">
                                                <a href="#"> New </a>
                                            </div>
                                            <figure class="images">
                                                <img src="images/65-inch.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $3,000.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> 65-Inch 4K Ultra HD Smart LED TV </a> 
                                            </div> 
                                            <div class="description">
                                                Comfort is a very important thing nowadays because it is a condition of satisfaction. 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active3"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>
                            </div><!--/item-->

                            <div class="item">
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left:  15px;">
                                            <div class="new">
                                                <a href="#"> New </a>
                                            </div>
                                            <figure class="images">
                                                <img src="images/28-inch.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $2,000.00 </h2> 
                                            </div>
                                            <div class="name">
                                                <a href="#">28-Inch 720p 60Hz LED TV</a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active4"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style=" margin-left: 15px;">
                                            <div class="new">
                                                <a href="#"> New </a>
                                            </div>
                                            <figure class="images">
                                                <img src="images/Soundbar.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $230.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#">Wireless Audio Soundbar</a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active5"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <div class="new">
                                                <a href="#"> New </a>
                                            </div>
                                            <figure class="images">
                                                <img src="images/blu-ray.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $350.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#">Blu-Ray Home Theater System</a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active6"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>
                            </div><!--/item-->
                        </div>
                    </div>
                </div>
            </div>
            <div class="clearfix"> </div>
            <!--Featured-->
            <div class="row">
                <div class="center-content">
                    <div class="row">
                        <div class="nav">
                            <h2> Featured </h2>
                            <ul>
                                <li><a href="#myFeatured" data-slide="prev"> <span class="glyphicon glyphicon-chevron-up"> </span></a></li>
                                <li><a href="#myFeatured" data-slide="next"> <span class="glyphicon glyphicon-chevron-down"> </span></a></li>
                            </ul>
                        </div>
                    </div>

                    <div id="myFeatured" class="carousel slide">
                        <div class="carousel-inner">
                            <div class="item active">
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <figure class="images">
                                                <img src="images/video-capture.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $90.00 </h2> <h3> $600.00 </h3>
                                            </div>
                                            <div class="name">
                                                <a href="#"> Wireless Smartphone Video Capture </a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active7"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style=" margin-left: 15px;">
                                            <figure class="images">
                                                <img src="images/65-inch.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $3,000.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> 65-Inch 4K Ultra HD Smart LED TV </a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active8"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <figure class="images">
                                                <img src="images/hard-floor.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $120.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> Hard Floor Expert Deluxe Canister </a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active9"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>
                            </div><!--/item-->

                            <div class="item">
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <figure class="images">
                                                <img src="images/ninja-blender.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $210.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#">Ninja Blender Duo with Auto-iQ</a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active10"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <figure class="images">
                                                <img src="images/48-inch.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $550.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> 48-Inch 1080p 60Hz Smart LED TV </a> 
                                            </div> 
                                            <div class="description">
                                                <p> Comfort is a very important thing nowadays because it is a condition of satisfaction. </p> 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active11"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>

                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="column" style="margin-left: 15px;">
                                            <figure class="images">
                                                <img src="images/65-inch.png" alt=""/>
                                            </figure>
                                            <div class="price">
                                                <h2> $3,000.00 </h2>
                                            </div>
                                            <div class="name">
                                                <a href="#"> 65-Inch 4K Ultra HD Smart LED TV </a> 
                                            </div> 
                                            <div class="description">
                                                Comfort is a very important thing nowadays because it is a condition of satisfaction. 
                                            </div>
                                            <div class="group-btn">
                                                <ul>
                                                    <li id="active12"><a href="#" class="glyphicon glyphicon-shopping-cart"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-heart"></a></li>
                                                    <li><a href="#" class="	glyphicon glyphicon-paperclip"></a></li>
                                                    <li><a href="#" class="glyphicon glyphicon-check"></a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>  
                                </div>
                            </div><!--/item-->
                        </div>
                    </div>
                </div>
            </div> 
        </div>
    </div>
</div>
</div>
</div>
</div>
</body>
</html>